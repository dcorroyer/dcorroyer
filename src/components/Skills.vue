<template>
  <section id="about" class="py-32 mb-20 md:mb-0">
    <div class="text-center">
      <h2 class="text-primary font-bold text-4xl md:text-3xl mb-14">
        Here <span class="text-black dark:text-white font-normal opacity-70">are my skills</span>
      </h2>
      <div class="flex flex-wrap items-center justify-center gap-24 md:gap-12">
        <div class="skill" data-text="Experience 3 years">
          <img
            class="w-16"
            src="/images/php.png"
            alt="php"
          />
        </div>
        <div class="skill" data-text="Experience 3 years">
          <img
            class="w-16"
            src="/images/symfony.png"
            alt="symfony"
          />
        </div>
        <div class="skill" data-text="Experience 2 years">
          <img
            class="w-16"
            src="/images/javascript.png"
            alt="javascript"
          />
        </div>
        <div class="skill" data-text="Some knowledge">
          <img
            class="w-16"
            src="/images/reactjs.png"
            alt="react"
          />
        </div>
        <div class="skill" data-text="Learning in progress">
          <img
            class="w-16"
            src="/images/vuejs.png"
            alt="vue"
          />
        </div>
        <div class="skill" data-text="Learning in progress">
          <img
            class="w-16"
            src="/images/nuxt.png"
            alt="nuxt"
          />
        </div>
        <div class="skill" data-text="Learning in progress">
          <img
            class="w-16"
            src="/images/tailwindcss.png"
            alt="tailwindcss"
          />
        </div>
        <div class="skill" data-text="Experience 2 years">
          <img
            class="w-16"
            src="/images/docker.png"
            alt="docker"
          />
        </div>
      </div>
    </div>
  </section>
</template>
